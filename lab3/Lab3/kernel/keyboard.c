...
{
	u8 scan_code = in_byte(KB_DATA); //KB_DATA的值为0x60,是读输出缓冲区和写输入缓冲区的端口

	if (kb_in.count < KB_IN_BYTES) {
		*(kb_in.p_head) = scan_code;
		kb_in.p_head++;
		if (kb_in.p_head == kb_in.buf + KB_IN_BYTES) { //如果缓冲区已经满，这里使用的策略是直接就把收到的字节丢弃
...
*======================================================================*/
/*======================================================================*
敲击键盘产生的编码被称作扫描码，分为Make Code和Break Code。当一个键被按下或保持住按
下时，将会产生Make Code，当键起时，产生Break Code
*======================================================================*/
PUBLIC void keyboard_read(TTY* p_tty)
{
	u8	scan_code;
	char	output[2];
	int	make;	/* 1: make;  0: break.  区分是Make Code还是Break Code*/
	...
	if(kb_in.count > 0){
        ...
        /* 下面开始解析扫描码 */
        //以下首先处理0xE1和0xE0,因为除了以0xE1和0xE0开头的扫描码，其余的都是单字节的
		if (scan_code == 0xE1) {
			int i;
